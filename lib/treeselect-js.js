...
class Treeselect {
        ...
        this.defaultArray = [
            {
                ...
                isGroup: false,
                level: 0,
                name: "AIML_Dialogflow"
            },
            {
                checked: true,
                childOf: "aiml_dialogflow",
                hidden: true,
                id: 1,
                isClosed: false,
                isGroup:  false,
    ...
    #createTreeselect() {
        ...
        if (this.flag === 0) {
            n.updateValue([s.flattedOptions[0]]),
                this.value = [1, 2, 3],
        ...
        n.srcElement.addEventListener("input", e=>{
            ...
            let temp_flag = false;
            e.detail.map((name)=>{
                s.flattedOptions.map((row) => {
                    if (row.childOf === name.id) {
        ...
        s.srcElement.addEventListener("input", e=>{
          ...
          /*  e = s;
            ...
            if (this.flag === 0){
                this.value = [1,2,3];
                s.map((row) => this.value.push(row.id));
                if (this.value[3] === 1){
                    this.flag = 1;
                }
                else{
                    this.defaultArray.map((row) => {
                        if(row.childOf !== 0){
                            s.push(row);
                        }
                    });
